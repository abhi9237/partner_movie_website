...
class DashBoardController extends GetxController {
  ...
  Future<void> getProductDetail(String id,{String? startDate, String? endDate}) async {
    isLoadingProductDetail.value = true;
    try {
      var response = await dashboardRepository.getProductDetail(id,startDate: startDate ??'',endDate:endDate ??'' );
      log("Response Data: $response");
      ...
      if (response.statusCode == 200) {
        ...
        if (result.watchMinutesGraph != null) {
          log(
            ' result.watchMinutesGraph!.toJson()==> ${result.watchMinutesGraph!.toJson()}',
          );
          weeklyWatchTimeList.value = convertToWeekdayMap(
            result.watchMinutesGraph!.toJson(),
          );
          log('weeklyWatchTimeList${weeklyWatchTimeList.value}');
        }
      } else {
        if (response.statusCode == 401) {
          final TokenInvalidResponse result = TokenInvalidResponse.fromJson(
  ...
  void selectDateRange(BuildContext context, String? id) async {
    ...
    if (picked != null && picked != selectedDateRange.value) {
      ...
      final int durationInDays = picked.end.difference(picked.start).inDays + 1;

      // 2. Check if the duration exceeds 30 days.
      if (durationInDays > 30) {
        // 3. If it does, show an alert dialog and do not proceed.
        showDialog(
          context: context,
          builder: (dialogContext) => AlertDialog(
            title:  Text('Invalid Date Range',style: appStyle(20),),
            content:
             Text('You cannot select a date range longer than 30 days.',style: appStyle(18),),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(dialogContext).pop(),
                child:  Text('OK',style: appStyle(20),),
              ),
            ],
          ),
        );
